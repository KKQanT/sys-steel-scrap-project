...
if __name__ == "__main__":
    ...
    for col in features:
      df_infer = df_infer.rename(columns = {col : f'd_DL2_month3_{col}'})

    infer_date = df_infer.loc[df_infer['target'].isna() == False]['target_date'].max()
    df_infer.loc[df_infer['target_date'] > infer_date, 'd_DL2_month3_pred'] = df_infer['predict']
    df_infer.loc[df_infer['target_date'] <= infer_date, 'd_DL2_month3'] = df_infer['predict']
