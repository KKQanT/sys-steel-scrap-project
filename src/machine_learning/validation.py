...
def walking_forward_fold(start_date, end_date, steps=7*12):
    test_min_date = start_date
    test_fold = {}
    cnt = 1
    while test_min_date <= end_date:
        test_max_date = test_min_date + dt.timedelta(days=steps)
        if test_max_date >= end_date:
            test_max_date = end_date
        test_fold[cnt] = (test_min_date, test_max_date)
        test_min_date+=dt.timedelta(days=steps)
        cnt+=1
    return test_fold